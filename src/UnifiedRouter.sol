...
contract UnifiedRouter is Ownable, ReentrancyGuard, IDclexSwapCallback, IUniswapV3SwapCallback {
    ...
    error UnifiedRouter__InvalidMsgValue();
    ...
    /// @dev Routes through dUSD as hub. Use address(0) for native ETH/DEL, wdel for wrapped
    function swapExactInput(
        ...
        if (inputToken == address(dUSD)) {
            ...
            // Native token - wrap to WDEL first, then swap
            if (msg.value != exactInput) revert UnifiedRouter__InvalidMsgValue();
            IWDEL(wdel).deposit{value: exactInput}();
            dUsdAmount = _swapV3ExactInput(wdel, address(dUSD), exactInput, address(this), address(this));
        } else if (inputToken == wdel) {
            // WDEL - swap via V3 WDEL/dUSD pool
        ...
        if (outputToken == address(dUSD)) {
            ...
            // Native token - swap to WDEL then unwrap
            output = _swapV3ExactInput(address(dUSD), wdel, dUsdAmount, address(this), address(this));
            IWDEL(wdel).withdraw(output);
            (bool success,) = msg.sender.call{value: output}("");
            if (!success) revert UnifiedRouter__NativeTransferFailed();
        } else if (outputToken == wdel) {
            // WDEL - swap via V3 dUSD/WDEL pool
    ...
    /// @dev Gets dUSD first, then executes exact-output swap for the output token
    function swapExactOutput(
        ...
        // For exact output, we need dUSD in the router BEFORE executing the output swap
        // The DclexPool callback will pull dUSD from the router

        if (outputToken == address(dUSD)) {
            // Output is dUSD - just get it from input
            if (inputToken == address(dUSD)) {
                // dUSD -> dUSD: no swap needed
                inputUsed = exactOutput;
                dUSD.safeTransferFrom(msg.sender, msg.sender, exactOutput);
            } else if (inputToken == address(0)) {
                // Native -> dUSD
                inputUsed = _swapV3ExactOutput(wdel, address(dUSD), exactOutput, msg.sender, address(this));
                if (msg.value < inputUsed) revert UnifiedRouter__InvalidMsgValue();
                IWDEL(wdel).deposit{value: inputUsed}();
            } else if (inputToken == wdel) {
                inputUsed = _swapV3ExactOutput(wdel, address(dUSD), exactOutput, msg.sender, msg.sender);
            } else {
                inputUsed = _swapToDusdExactOutput(inputToken, exactOutput, maxInput, pythData);
                // Transfer dUSD to user (it went to router)
                dUSD.safeTransfer(msg.sender, exactOutput);
            }
        } else {
            // Output is NOT dUSD - need to get dUSD first, then swap to output
            // Step 1: Get dUSD into the router from input
            uint256 dUsdInRouter;
            if (inputToken == address(dUSD)) {
                // Pull max from user, refund excess later
                dUSD.safeTransferFrom(msg.sender, address(this), maxInput);
                dUsdInRouter = maxInput;
                inputUsed = 0; // Will be calculated after output swap
            } else if (inputToken == address(0)) {
                // Native -> dUSD: wrap and swap
                if (msg.value == 0) revert UnifiedRouter__InvalidMsgValue();
                IWDEL(wdel).deposit{value: msg.value}();
                dUsdInRouter = _swapV3ExactInput(wdel, address(dUSD), msg.value, address(this), address(this));
                inputUsed = msg.value;
            } else if (inputToken == wdel) {
                // Pull max WDEL, swap to dUSD
                IERC20(wdel).safeTransferFrom(msg.sender, address(this), maxInput);
                dUsdInRouter = _swapV3ExactInput(wdel, address(dUSD), maxInput, address(this), address(this));
                inputUsed = maxInput;
            } else {
                // Sell input token for dUSD
                inputUsed = _swapToDusdExactOutput(inputToken, maxInput, maxInput, pythData);
                dUsdInRouter = dUSD.balanceOf(address(this));
            }

            // Step 2: Execute exact-output swap for the output token
            uint256 dUsdUsed;
            if (outputToken == address(0)) {
                // dUSD -> Native: swap to WDEL then unwrap
                dUsdUsed = _swapV3ExactOutput(address(dUSD), wdel, exactOutput, address(this), address(this));
                IWDEL(wdel).withdraw(exactOutput);
                (bool success,) = msg.sender.call{value: exactOutput}("");
                if (!success) revert UnifiedRouter__NativeTransferFailed();
            } else if (outputToken == wdel) {
                dUsdUsed = _swapV3ExactOutput(address(dUSD), wdel, exactOutput, msg.sender, address(this));
            } else {
                dUsdUsed = _swapFromDusdExactOutput(outputToken, exactOutput, pythData);
            }

            // Calculate actual input used (for dUSD input case)
            if (inputToken == address(dUSD)) {
                inputUsed = dUsdUsed;
            }

            // Refund excess dUSD
            uint256 dUsdRemaining = dUSD.balanceOf(address(this));
            if (dUsdRemaining > 0) {
                dUSD.safeTransfer(msg.sender, dUsdRemaining);
            }
        }
    ...
    function uniswapV3SwapCallback(
        ...
        // Derive owed token from pool - positive delta means we owe that token
        uint256 amountToPay;
        address tokenToPay;
        if (amount0Delta > 0) {
            amountToPay = uint256(amount0Delta);
            tokenToPay = IUniswapV3Pool(msg.sender).token0();
        } else {
            amountToPay = uint256(amount1Delta);
            tokenToPay = IUniswapV3Pool(msg.sender).token1();
        }
    ...
    /// @notice Swap dUSD (already in router) for exact output of token
    /// @dev Requires dUSD to be in the router before calling
    function _swapFromDusdExactOutput(
        address token,
        ...
        DclexPool dclexPool = dclexPools[token];
        if (address(dclexPool) != address(0)) {
            // Router pays dUSD from its balance
            return dclexPool.swapExactOutput(
                true,
                outputAmount,
                msg.sender,
                abi.encode(DclexSwapCallbackData(address(this), false, address(0), 0)),
        ...
        if (v3Pool != address(0)) {
            // Router pays dUSD from its balance
            return _swapV3ExactOutput(address(dUSD), token, outputAmount, msg.sender, address(this));
        }
    ...
    function _quoteV3ExactOutput(
        ...
        // Simplified quote using FullMath to avoid overflow
        // In production, consider using the V3 Quoter contract for accuracy
        address pool = _getV3Pool(tokenIn, tokenOut);
        (uint160 sqrtPriceX96,,,,,,) = IUniswapV3Pool(pool).slot0();

        bool zeroForOne = tokenIn < tokenOut;
        // price = (sqrtPriceX96)^2 / 2^192, use FullMath for safe multiplication
        uint256 priceX192 = FullMath.mulDiv(uint256(sqrtPriceX96), uint256(sqrtPriceX96), 1);

        if (zeroForOne) {
            // token0 -> token1: need more token0 for given token1
            return FullMath.mulDiv(amountOut, 1 << 192, priceX192);
        } else {
            // token1 -> token0: need more token1 for given token0
            return FullMath.mulDiv(amountOut, priceX192, 1 << 192);
        }
    }
    ...
    function _getV3Pool(address tokenA, address tokenB) internal view returns (address) {
        // Only use registered pools to ensure callback validation works
        address pool = v3Pools[tokenA];
        if (pool != address(0) && _validV3Pools[pool]) return pool;
