...
/// @title Mock V3 Pool for testing
contract MockV3Pool {
    address public token0;
    address public token1;
    uint160 public sqrtPriceX96;
    uint256 public price; // simplified price for mock

    constructor(address _token0, address _token1, uint256 _price) {
        // Ensure token0 < token1 as per Uniswap convention
        if (_token0 < _token1) {
            token0 = _token0;
            token1 = _token1;
        } else {
        ...
        price = _price;
        // Set a reasonable sqrtPriceX96 (price = 1 means sqrtPriceX96 = 2^96)
        sqrtPriceX96 = uint160(79228162514264337593543950336); // ~1:1 price
    }
    ...
    function swap(
        ...
        if (amountSpecified > 0) {
            ...
            uint256 amountIn = amountOut * 1e18 / price + 1; // Round up
    ...
    // Helper to fund the pool for testing
...
contract UnifiedRouterTest is Test {
    ...
    // ============ Cross-Pool Swap Tests ============

    function testSwapExactInput_dUsdToStock() public {
        ...
        assertGt(aaplAfter, aaplBefore, "Should receive AAPL");
        assertEq(dUsdBefore - dUsdAfter, inputAmount, "Should spend exact dUSD");
    }

    function testSwapExactInput_stockToDusd() public {
        ...
        assertEq(aaplBefore - aaplAfter, inputAmount, "Should spend exact AAPL");
        assertGt(dUsdAfter, dUsdBefore, "Should receive dUSD");
    }

    function testSwapExactInput_stockToStock() public {
        // Swap AAPL -> NVDA (via dUSD hub)
        uint256 inputAmount = 5 ether;
        ...
        uint256 aaplAfter = aaplStock.balanceOf(USER_1);
        uint256 nvdaAfter = nvdaStock.balanceOf(USER_1);

        vm.stopPrank();

        assertEq(aaplBefore - aaplAfter, inputAmount, "Should spend exact AAPL");
        assertGt(nvdaAfter, nvdaBefore, "Should receive NVDA");
    }

    function testSwapExactOutput_dUsdToStock() public {
        uint256 outputAmount = 5 ether;
        uint256 maxInput = 2000e18;
        ...
        assertEq(aaplAfter - aaplBefore, outputAmount, "Should receive exact AAPL");
    }

    // ============ Callback Validation Tests ============

    function testRevert_invalidDclexCallback() public {
        // Non-pool address calling callback should revert
        vm.expectRevert(UnifiedRouter.UnifiedRouter__NotDclexPool.selector);
        router.dclexSwapCallback(
            address(dUSD),
            1000e18,
            abi.encode(UnifiedRouter.DclexSwapCallbackData(USER_1, false, address(0), 0))
        );
    }

    function testRevert_invalidV3Callback() public {
        // Non-pool address calling callback should revert
        vm.expectRevert(UnifiedRouter.UnifiedRouter__NotV3Pool.selector);
        router.uniswapV3SwapCallback(
            1000,
            -1000,
            abi.encode(UnifiedRouter.V3CallbackData(address(dUSD), address(wdel), 3000, USER_1))
        );
    }

    // ============ Fuzz Tests for Cross-Pool ============

    function testFuzz_swapExactInput_stockToStock(uint256 inputAmount) public {
        inputAmount = bound(inputAmount, 1 ether, 50 ether);
        ...
        assertEq(aaplBefore - aaplAfter, inputAmount, "Should spend exact input");
        assertGt(nvdaAfter, nvdaBefore, "Should receive output");
    }

    // ============ Input Validation Tests ============

    function testRevert_inputTooHigh() public {
        uint256 outputAmount = 5 ether;
        uint256 maxInput = 100e18; // Very low max, will be exceeded

        vm.startPrank(USER_1);
        dUSD.approve(address(router), 10000e18);

        vm.expectRevert(UnifiedRouter.UnifiedRouter__InputTooHigh.selector);
        router.buyExactOutput(
            address(aaplStock),
            outputAmount,
            maxInput,
