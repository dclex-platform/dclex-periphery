...
///      The dUSD stablecoin must be registered on Factory beforehand
///      (`Factory.createStablecoin("dUSD", "dUSD")`).
contract BatchPoolInitializer {
    string constant DUSD_SYMBOL = "dUSD";
    uint256 constant STOCK_AMOUNT = 1000e18; // 1000 stock tokens (18 decimals)
    uint256 constant DUSD_AMOUNT = 10_000e6; // 10,000 dUSD (6 decimals)
    ...
    function _initializePool(
        ...
        address dusdAddress = address(pool.usdcToken());
        ...
        factory.forceMintStablecoin(DUSD_SYMBOL, address(this), DUSD_AMOUNT);
        factory.forceMintStocks(symbol, address(this), STOCK_AMOUNT);

        IERC20(stockAddress).approve(poolAddress, STOCK_AMOUNT);
        IERC20(dusdAddress).approve(poolAddress, DUSD_AMOUNT);
        pool.initialize{value: pythFee}(STOCK_AMOUNT, DUSD_AMOUNT, priceUpdateData);
    }
